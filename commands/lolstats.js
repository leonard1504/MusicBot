...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (rank.response.length > 0 && rank.response[0] != undefined) {
            switch (rank.response[0].tier) {
                case "IRON":
                    soloranksymbol = `${iron}`;
                    break;
                case "BRONZE":
            ...
            wlratiosolo = Math.round((rank.response[0].wins / (rank.response[0].wins + rank.response[0].losses)) * 100);
            if (rank.response[0].queueType === "RANKED_SOLO_5x5") {
                solostring = `**${soloranksymbol} ${rank.response[0].tier} ${rank.response[0].rank}** \n LP: ${rank.response[0].leaguePoints} \n Gewonnen: ${rank.response[0].wins} \n Verloren: ${rank.response[0].losses} \n Winrate: ${wlratiosolo}%`;
            } else if (rank.response[0].queueType === "RANKED_FLEX_SR") {
                flexstring = `**${soloranksymbol} ${rank.response[0].tier} ${rank.response[0].rank}** \n LP: ${rank.response[0].leaguePoints} \n Gewonnen: ${rank.response[0].wins} \n Verloren: ${rank.response[0].losses} \n Winrate: ${wlratiosolo}%`;
            }
        } else {
            solostring = `${leaveemoji} Keine Ranglistendaten gefunden`;
        }

        if (rank.response.length > 0 && rank.response[1] != undefined) {
            switch (rank.response[1].tier) {
                case "IRON":
                    flexranksymbol = `${iron}`;
                    break;
                case "BRONZE":
            ...
            if (rank.response[1].queueType === "RANKED_FLEX_SR") {
                flexstring = `**${flexranksymbol} ${rank.response[1].tier} ${rank.response[1].rank}** \n LP: ${rank.response[1].leaguePoints} \n Gewonnen: ${rank.response[1].wins} \n Verloren: ${rank.response[1].losses} \n Winrate: ${wlratioflex}%`;
            } else if (rank.response[1].queueType === "RANKED_SOLO_5x5") {
                solostring = `**${flexranksymbol} ${rank.response[1].tier} ${rank.response[1].rank}** \n LP: ${rank.response[1].leaguePoints} \n Gewonnen: ${rank.response[1].wins} \n Verloren: ${rank.response[1].losses} \n Winrate: ${wlratioflex}%`;
            }
        } else {
        ...
        try {
            ...
            if (Object.keys(Object.values(getKDA)[0]).includes("400")) {
                ...
                normallanes = `${interaction.options.get('spielername').value} hat in Normals \n **${gamemodes[indexN].lanes.top.n}** Spiele als **Toplaner** gespielt \n **${gamemodes[indexN].lanes.jungle.n}** Spiele als **Jungler** gespielt \n **${gamemodes[indexN].lanes.middle.n}** Spiele als **Midlaner** gespielt \n **${gamemodes[indexN].lanes.bottom.n}** Spiele als **ADC** gespielt \n **${gamemodes[indexN].lanes.support.n}** Spiele als **Supporter** gespielt`;
                normalschampid = [];
                champnames = [];
